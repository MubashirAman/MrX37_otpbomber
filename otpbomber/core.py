...
def credit():
    """Print Credit"""
    print('LinkedIn : www.linkedin.com/in/mubashir-aman-918632282')
    print('Github : https://github.com/MubashirAMAN/otpbomber')
    print('Repositry Structure Inspired by https://github.com/asharbinkhalil/otpbomber')
...
async def maincore():
    ...
    parser.add_argument("--count", "--loops", default=1, required=False, action="store", dest="count", type=int,
                        help="Number of iterations to run for each module (default: 1)")
    ...
    # normalize loop count to a safe positive integer (added loop handling)
    try:
        loops = int(args.count)
        if loops < 1:
            loops = 1
    except Exception:
        loops = 1
    ...
    # repeat websites list according to loops (so existing nursery loop runs each iteration)
    if loops > 1:
        websites = websites * loops
    ...
    trio.lowlevel.add_instrument(instrument)  # Add instrument once here
